...
# Note: obtained executing DepQBF
...
##############################################################################################
### FILTERED INSTANCES FOR ITERATIVE EXPERIMENTATION #########################################

# Avoid instances that DepQBF can not handle
# Avoid instances that are too small (less than 100 variables or 100 clauses)
...
##############################################################################################
### INSTANCES THAT ARE SAT INSTANCES (EVERY VARIABLE IS EXISTENTIAL) #########################
...
##############################################################################################
### FUNCTIONS TO RUN SOLVERS AND MEASURE USED TIME AND MEMORY ################################

def run_solver_psutil(solver_path: str, instance_path: str, timeout_seconds: Union[int, float], python=False) -> Dict:
    """
    Runs a QBF solver on an instance and collects these metrics:
    
    * Instance name
    * Solver output: SAT or UNSAT
    * Status: CORRECT, INCORRECT, TIMEOUT or ERROR
    * Total wall time: FALSE metric of the wall/real time spent in the resolution (affected by time.sleep, see 
        peak memory in MB). Fixed with the next function
    * Total cpu time
    * Peak memory in MB: its an approximation, since it depends in the time between the communications between the 
        process executing this function and the solvers (100 ms)

    Args:
        solver_path (str): path corresponding to the QBF solver to be used (command line name, python script, personally built binary executable...)
        instance_path (str): path to the instance to be solved
        timeout_seconds (int | float): the maximum time that the solver has to finish the resolution
        python (bool): flag that indicates if the solver is implemented in a Python script
...
def run_solver_walltime(solver_path: str, instance_path: str, timeout_seconds: Union[int, float], python=False) -> Dict:
    """
    Runs a QBF solver on an instance and collects the spent wall time along the following information:
    
    * Instance name
    * Solver output: SAT or UNSAT
    * Status: CORRECT, INCORRECT, TIMEOUT or ERROR
    * Total wall time: wall/real time spent in the resolution (no use of time.sleep)
...
if __name__ == "__main__":
    ...
    # Note: if the directory in the server of the solver scripts is different change this
    #solver_name, solver_path, python = "DepQBF", "depqbf", False
    #solver_name, solver_path, python = "Naive", "src/qbf_naive_solver.py", True
    if iterative_experimentation:
        # Note: change _iter for _cached, _tuples or _final
        solver_name, solver_path, python = "FNI_iter", "src/qbf_inf_solver_iter.py", True
    else:
