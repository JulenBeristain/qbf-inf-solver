...
def _eliminate_variables(quantifiers: List[QBlock], ccnf: Union[int, bool], config: Dict[str, bool]) -> bool:
    if config['iterative']:
        return _eliminate_variables_it(quantifiers, ccnf, config)
    return _eliminate_variables_rec(quantifiers, ccnf, config)

def _eliminate_variables_rec(quantifiers: List[QBlock], ccnf: Union[int, bool], config: Dict[str, bool]) -> bool:
    """
    ...
    if q == 'a':
        ...
        if config['elim_e_disj_conj']:
            ...
            if debugging: print("Disyunción...")
            psi = CCNF.disjunction(psi1, psi2, config)
...
def _eliminate_variables_it(quantifiers: List[QBlock], ccnf: Union[int, bool], config: Dict[str, bool]) -> bool:
    """
    TODO: microoptimización, una vez testeados las variantes, quedarnos con la más eficiente y quitar los flags y comprobaciones
    """
    prev_num_nodes = 0
